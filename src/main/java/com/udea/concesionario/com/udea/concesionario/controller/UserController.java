...
import com.udea.concesionario.com.udea.concesionario.controller.util.Page;
import com.udea.concesionario.com.udea.concesionario.controller.util.PaginationUtil;
import static com.udea.concesionario.config.Constants.EMAIL_ALREADY_USED_TYPE;
import static com.udea.concesionario.config.Constants.LOGIN_ALREADY_USED_TYPE;
...
public class UserController {
    ...
    public Response createUser(ManagedUserVM managedUserVM) throws URISyntaxException {
        ...
        if (userFacade.findOneByLogin(managedUserVM.getLogin().toLowerCase()).isPresent()) {
            return HeaderUtil.createFailureAlert(Response.status(BAD_REQUEST), "userManagement", "userexists", LOGIN_ALREADY_USED_TYPE).build();
        } else if (userFacade.findOneByEmail(managedUserVM.getEmail()).isPresent()) {
    ...
    public Response updateUser(ManagedUserVM managedUserVM) {
        ...
        Optional<User> existingUser = userFacade.findOneByEmail(managedUserVM.getEmail());
        if (existingUser.isPresent() && (!existingUser.get().getId().equals(managedUserVM.getId()))) {
            return HeaderUtil.createFailureAlert(Response.status(BAD_REQUEST), "userManagement", "emailexists", EMAIL_ALREADY_USED_TYPE).build();
        }
        existingUser = userFacade.findOneByLogin(managedUserVM.getLogin().toLowerCase());
        if (existingUser.isPresent() && (!existingUser.get().getId().equals(managedUserVM.getId()))) {
            return HeaderUtil.createFailureAlert(Response.status(BAD_REQUEST), "userManagement", "userexists", LOGIN_ALREADY_USED_TYPE).build();
        }

...
import static com.udea.concesionario.config.Constants.EMAIL_NOT_FOUND_TYPE;
import static com.udea.concesionario.config.Constants.INVALID_PASSWORD_TYPE;
...
public class AccountController {
    ...
    public Response registerAccount(@Valid ManagedUserVM managedUserVM) {
        ...
        return userFacade.findOneByLogin(managedUserVM.getLogin().toLowerCase())
                .map(user -> Response.status(BAD_REQUEST).type(TEXT_PLAIN).entity(LOGIN_ALREADY_USED_TYPE).build())
                .orElseGet(() -> userFacade.findOneByEmail(managedUserVM.getEmail())
